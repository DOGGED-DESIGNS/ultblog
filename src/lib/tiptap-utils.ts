...
export const handleImageUpload = async (
  ...
  if (file.size > MAX_FILE_SIZE) {
    throw new Error(
      `File size exceeds maximum allowed (${MAX_FILE_SIZE / (4 * 1024 * 1024)}MB)`,
    );
  }

  const compressed = await imageCompression(file, {
    maxSizeMB: 1, // target max size (in MB)
    maxWidthOrHeight: 1600, // prevent huge dimensions
    useWebWorker: true,
    fileType: "image/webp", // force webp
    // onProgress: (p) => onProgress?.({ progress: p }),
  });
  ...
  try {
    console.log(`file type oo ${webpFile.type}`);

    const hash = await hashFile(webpFile);

    const res = await fetch("/api/image-exists", {
      method: "POST",
    ...
    // 3️⃣ If it already exists → skip UploadThing 🎯
    if (exists && url) {
      onProgress?.({ progress: 100 });
      return url;
    }

    const upload = await uploadFiles("imageUploader", {
      files: [webpFile],
      input: { hash },
      onUploadProgress: ({ progress }) =>
        onProgress?.({ progress: Math.round(progress / 10) * 10 }),
    });

    if (!upload?.[0]) throw new Error("upload fail");
    onProgress?.({ progress: 100 });
    return upload[0].ufsUrl;
  } catch (err) {
    if (abortSignal?.aborted) throw new Error("upload Canclled");
    throw err;
  }
